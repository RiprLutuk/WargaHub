...
<template>
  <div class="container page-stack">
    <header class="page-heading"><span class="eyebrow">Sumber resmi</span><h1>Pengumuman lingkungan</h1><p>Informasi penting tersusun rapi dan dapat dicari kembali kapan pun Anda membutuhkannya.</p></header>
    ...
    <div v-else class="announcement-list">
      <article v-for="item in filtered" :id="item.id" :key="item.id" class="card card-body announcement-row">
        <div><div class="meta"><span>{{ item.category }}</span><time :datetime="item.publishedAt">{{ formatDate(item.publishedAt) }}</time></div><h2>{{ item.title }}</h2><p>{{ item.summary }}</p></div>
...
<style scoped>
.filter-bar { display: grid; grid-template-columns: 1fr minmax(12rem, .3fr); gap: .75rem; padding: .8rem; border: 1px solid var(--line); border-radius: var(--radius-md); background: var(--paper); }
.search-box { display: flex; align-items: center; gap: .55rem; padding-inline: .75rem; border: 1px solid var(--line-strong); border-radius: .72rem; background: white; color: var(--ink-500); }
...
.announcement-row { display: flex; align-items: flex-start; justify-content: space-between; gap: 1rem; scroll-margin-top: 6rem; }
.announcement-row h2 { margin-bottom: .45rem; font-size: 1.28rem; }
.announcement-row p { max-width: 50rem; margin-bottom: 0; color: var(--ink-650); }
.meta { display: flex; gap: .7rem; margin-bottom: .5rem; color: var(--ink-650); font-size: .74rem; font-weight: 750; }
.priority { flex: none; padding: .3rem .55rem; border-radius: 999px; background: var(--amber-100); color: var(--amber-700); font-size: .7rem; font-weight: 850; }
