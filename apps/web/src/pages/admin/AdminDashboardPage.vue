...
import { Activity, AlertTriangle, ArrowRight, Banknote, CheckCircle2, Clock3, FileText, Home, ReceiptText, Users } from 'lucide-vue-next';
  ...
  <div class="admin-page">
    <header class="admin-heading">
      <div>
        ...
        <p>Prioritaskan pekerjaan yang menunggu tindakan. Metrik disajikan secara agregat tanpa ranking warga.</p>
      </div>

      <div class="heading-actions">
        <button class="button button-sm" type="button" @click="waModalOpen = true">
          <FileText :size="15" /> Impor Teks Pesan
        </button>
...
.last-update { display: inline-flex; align-items: center; gap: 0.35rem; padding: 0.4rem 0.6rem; border-radius: 999px; background: var(--success-100); color: var(--success-700); font-size: 0.72rem; font-weight: 800; }
.metric-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.75rem; }
.metric-grid article { display: flex; align-items: center; gap: 0.8rem; padding: 1rem; border: 1px solid var(--line); border-radius: var(--radius-md); background: var(--paper); box-shadow: var(--shadow-sm); }
.metric-icon { display: grid; width: 2.7rem; height: 2.7rem; flex: none; place-items: center; border-radius: 0.8rem; }
...
.metric-grid em { overflow: hidden; color: var(--ink-500); font-size: 0.66rem; font-style: normal; text-overflow: ellipsis; white-space: nowrap; }
.admin-columns { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; }
.queue-list { display: grid; }
.queue-list article { display: flex; align-items: center; gap: 0.65rem; padding: 0.7rem 0; border-bottom: 1px solid var(--line); }
.queue-list article:last-child { border: 0; }
.queue-list article > div:nth-child(2) { min-width: 0; flex: 1; }
.queue-list h3 { margin: 0; overflow: hidden; font-size: 0.88rem; text-overflow: ellipsis; white-space: nowrap; }
.queue-list p { margin: 0; color: var(--ink-650); font-size: 0.72rem; }
.avatar { display: grid; width: 2.25rem; height: 2.25rem; place-items: center; border-radius: 0.65rem; background: var(--cream-100); color: var(--ink-800); font-weight: 850; }
.metric-icon.small { width: 2.25rem; height: 2.25rem; }
.quick-actions > div { display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.75rem; }
.quick-actions a, .action-card { display: flex; align-items: center; gap: 0.7rem; padding: 1rem; border: 1px solid var(--line); border-radius: var(--radius-md); background: var(--paper); color: var(--ink-950); text-decoration: none; text-align: left; cursor: pointer; }
